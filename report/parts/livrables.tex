...
Les seuls points un peu exotiques de ce code sont les filtres (qui
sont ici choisis comme des filtres laissant tout passer). Le but des
filtres est de pouvoir choisir quel(s) type(s) d'arêtes seront
utilisés dans les différents parcours et recherche du plus court
chemin. Un filtre NULL laisse toutes les arêtes passer, un exemple sur
l'utilisation des filtres se trouve dans le fichier
\texttt{src/examples/main\_filters.cc}.
...
\item Une interface console où la majorité des fonctionalités de l'API
  peuvent être testées à la main.
\end{itemize} 
